...
- **-DUSE_MESAZINK:BOOL=\<bool\>** controls whether Mesa Zink DLL overrides (`libgallium_wgl.dll`, `opengl32.dll`, `libglapi.dll`) are bundled in 64bit Windows builds. Default is `OFF`. Set this to `ON` only when testing Mesa/Zink behavior.
...
- To build a 64bit viewer with Mesa Zink DLL overrides enabled for testing, run this command:
`autobuild configure -A 64 -c ReleaseFS_open -- --chan MyViewer -DLL_TESTS:BOOL=FALSE -DUSE_MESAZINK:BOOL=ON`
