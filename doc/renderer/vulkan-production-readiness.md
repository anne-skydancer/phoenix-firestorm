...
| P0e4 recursive/offscreen views | In progress; deterministic contract and P0e4a authenticated production packet accepted on revision `a34aa5c74087a19c3bcf19dcda4d8f4cb56d6554` | The validated `p0e4a-production-capture-manifest.json` preserves an authenticated 11-pass packet: one impostor, dynamic texture, preview, pre-water alpha, and activated media surface plus all six canonical 360 cube faces under one generation. Complete bounded private peer replay, image evidence, and the real reflection/exclusion resources required by P0e2. |
