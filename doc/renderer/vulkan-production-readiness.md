...
| P0e4 recursive/offscreen views | In progress; deterministic contract and authenticated production packet accepted on the P0.2 working tree | Packet v2 inspection passed for an authenticated 11-pass production capture: one impostor, dynamic texture, preview, pre-water alpha, and activated media surface plus all six canonical 360 cube faces under one generation. Commit the implementation and issue its immutable manifest, then complete bounded private peer replay, image evidence, and the real reflection/exclusion resources required by P0e2. |
