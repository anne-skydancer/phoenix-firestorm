/**
 * @file llghiopenglmaterialskinharness.cpp
 * @brief Native OpenGL execution harness for the R5a material/skin fixture.
 *
...
void report(const LL::GHI::Test::MaterialSkinFixtureResult& result, const char* profile)
{
...
int main()
{
    ...
    const wchar_t* className = L"VulkanstormR5aOpenGLHarness";
    ...
    if (!RegisterClassW(&windowClass) && GetLastError() != ERROR_CLASS_ALREADY_EXISTS)
        return 2;
    HWND window = CreateWindowExW(0, className, L"R5a OpenGL material skin",
    ...
    else
    {
        ...
        {
            auto result = LL::GHI::Test::runMaterialSkinFixture(*creation.device, package);
            if (!result.passed)
            {
            ...
            {
                auto fallbackResult =
                    LL::GHI::Test::runMaterialSkinFixture(*creation.device, fallback);
