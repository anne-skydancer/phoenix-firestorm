/**
 * @file llghiopenglworldharness.cpp
 * @brief Native OpenGL execution harness for the R5c-e world fixture.
 *
...
#include "tests/ghi/llghiworldfixture.h"
...
#ifndef LL_GHI_R5_WORLD_SHADER_PACKAGE
#error LL_GHI_R5_WORLD_SHADER_PACKAGE must name the packaged R5 world shader
#endif
...
void report(const LL::GHI::Test::WorldFixtureResult& result, const char* profile)
{
    std::cout << result.message << " backend=OpenGL profile=" << profile
              << " depth-stencil=" << static_cast<int>(result.depthStencilFormat);
...
int main()
{
    ...
    const wchar_t* className = L"VulkanstormR5WorldOpenGLHarness";
    ...
    if (!RegisterClassW(&windowClass) && GetLastError() != ERROR_CLASS_ALREADY_EXISTS)
        return 2;
    HWND window = CreateWindowExW(0, className, L"R5 OpenGL world",
    ...
    LL::GHI::Status status = LL::GHI::loadShaderPackage(
        LL_GHI_R5_WORLD_SHADER_PACKAGE, package);
    int exitCode = 0;
    if (!status)
    {
    ...
    else
    {
        ...
        {
            auto result = LL::GHI::Test::runWorldFixture(*creation.device, package);
            if (!result.passed)
            {
            ...
            if (!exitCode)
            {
                auto fallbackResult = LL::GHI::Test::runWorldFixture(*creation.device, fallback);
