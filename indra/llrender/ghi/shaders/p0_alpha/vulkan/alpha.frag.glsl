...
layout(set = 3, binding = 0, r32ui) uniform uimage2D ppllHead;
layout(set = 3, binding = 1, std430) buffer PPLLNodes { uvec4 nodes[]; } nodeData;
layout(set = 3, binding = 2, std430) buffer PPLLCounter { uint nextNode; uint overflowCount; } counterData;
...
void main()
{
    if (alphaData.ppllConfig.x == 2u)
    {
        ...
        uint traversal = 0u; uint exactLimit = clamp(alphaData.ppllConfig.z, 1u, MAX_EXACT_LAYERS);
        while (node != INVALID_NODE && node < alphaData.ppllConfig.y && traversal < 256u)
        {
    ...
    bool replay = alphaData.ppllConfig.x == 3u;
    if (replay)
    {
    ...
    float diffuse = max(dot(normalize(worldNormal),
        normalize(-alphaData.lightDirectionMode.xyz)), 0.0);
    vec3 lighting = alphaData.ambientMinimumAlpha.rgb
                  + alphaData.directionalModel.rgb * diffuse;
    ...
    if (alphaData.ppllConfig.x == 1u)
    {
        uint node = atomicAdd(counterData.nextNode, 1u);
        if (node >= alphaData.ppllConfig.y)
        { atomicAdd(counterData.overflowCount, 1u); outColor = vec4(color, base.a); return; }
        ivec2 pixel = ivec2(gl_FragCoord.xy);
        uint previous = imageAtomicExchange(ppllHead, pixel, node);
        nodeData.nodes[node] = uvec4(packHalf2x16(color.rg), packHalf2x16(vec2(color.b, base.a)), floatBitsToUint(gl_FragCoord.z), previous);
