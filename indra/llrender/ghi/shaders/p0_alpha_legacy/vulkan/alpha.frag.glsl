...
{
    vec4 lightDirectionMode; vec4 ambientMinimumAlpha; vec4 directionalModel;
} alphaData;
layout(set = 2, binding = 0, std140) uniform MaterialData
{
    vec4 baseColorFactor; vec4 emissiveMetallic; vec4 roughnessNormalScale;
    vec4 materialParams; vec4 uvOffsetScale[4]; vec4 uvRotation[4];
} materialData;
...
void main()
{
    ...
    if (alphaData.lightDirectionMode.a > 0.5)
    {
        ...
        outColor = vec4(0.0, 0.0, 0.0, glow); return;
    }
    ...
    if (!legacy) color += texture(emissiveMap, transformedUv(3u)).rgb
