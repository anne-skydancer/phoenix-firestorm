...
layout(set = 0, binding = 0, std140) uniform PeelData
{
    uvec4 peelConfig;
    vec4 opaqueDepth;
} peelData;
layout(set = 0, binding = 1) uniform sampler2D priorDepth;
...
void main()
{
    ...
    bool behindPrevious = peelData.peelConfig.x == 0u ||
        depth < previous - 0.00001;
    bool eligible = depth >= peelData.opaqueDepth.x && behindPrevious;
