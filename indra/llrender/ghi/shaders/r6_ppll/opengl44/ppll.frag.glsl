...
layout(std140, binding = 0) uniform AlphaData
{
    uvec4 alphaConfig;
    vec4 opaqueDepth;
};
...
void captureFragment()
{
    ...
    if (node >= alphaConfig.y)
    {
        ...
        // The capture pipeline uses straight-alpha blending. Overflow stays in
        // the color attachment as the deterministic residual tail.
...
void resolveFragment()
{
    ...
    while (node != INVALID_NODE && node < alphaConfig.y && traversal < 256u)
    {
        ...
        // GHI uses reverse-Z: greater window depth is nearer. Fragments behind
        // the opaque surface therefore have a smaller depth and are rejected.
    ...
    // Ascending reverse-Z depth is back-to-front.
