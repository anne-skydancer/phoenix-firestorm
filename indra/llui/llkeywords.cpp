                                      ...
                                      LLStyleConstSP style)
{
    ...
    if (op_index == 0)
    {
        ...
        LLStyleSP base_style = getDefaultStyle(editor);
        base_style->setColor(style->getColor());
        seg_list->push_back( new LLNormalTextSegment( base_style, 0, text_len, editor ) );
    }
    ...
    for (; op_index < end_index; ++op_index)
    {
        ...
        {
            LLTextSegmentPtr text_segment = new LLLineBreakTextSegment(getDefaultStyle(editor), op.start);
