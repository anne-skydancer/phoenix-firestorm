...
#include <algorithm>
#include <utility>
#include "llpreviewscript.h"   // LLScriptEdCore for F1 help (FS)
...
const S32   UI_TEXTEDITOR_LINE_NUMBER_MARGIN = 40;   // FS FIRE-23047
...
void LLScriptEditor::drawLineNumbers()
{
    ...
    if (mShowLineNumbers)
    {
        ...
        gl_rect_2d(UI_TEXTEDITOR_LINE_NUMBER_MARGIN, top, UI_TEXTEDITOR_LINE_NUMBER_MARGIN-1, bottom, LLColor4::grey3 % getCurrentTransparency()); // separator  FS FIRE-6955
...
{
    LL_PROFILE_ZONE_SCOPED;
    if (getKeywords().loadFromLegacyFile(filename))
    {
        auto count = llmin(funcs.size(), tooltips.size());
        for (size_t i = 0; i < count; i++)
        {
            std::string name = utf8str_trim(funcs[i]);
            getKeywords().addToken(LLKeywordToken::TT_WORD, name, LLUIColor(color), tooltips[i] );
        }
        segment_vec_t segment_list;
