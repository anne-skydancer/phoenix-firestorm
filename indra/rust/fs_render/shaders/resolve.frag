...
layout(set = 0, binding = 0) uniform Env {
    ...
    vec4 bg;        // rgb viewer clear/background for non-lit pixels
} env;
...
void main() {
    ...
    // Not lit world geometry (cleared background): emit the background. The resolve runs
    // BEFORE the forward pass, which then draws sky/terrain/water over this with depth-test.
    if (abs(a.a - HAS_ATMOS) > 0.1) {
        frag = vec4(env.bg.rgb, 1.0);
        return;
    }
