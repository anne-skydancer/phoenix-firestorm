...
#define MAX(a,b) ((a) > (b)) ? (a) : (b)
	...
	void Soloud::update3dVoices_internal(unsigned int *aVoiceArray, unsigned int aVoiceCount)
	{
		...
		for (i = 0; i < (signed)aVoiceCount; i++)
		{
			...
			// <FS> Discrete-5.1/7.1 tuning (SoLoud's stock panner targets
			// stereo; on separated speakers it bleeds and mishandles the LFE).
			// LFE_LEVEL: the sub is a null-vector speaker; stock code sends
			//   every voice to it at full range/full volume (boomy mush on
			//   real subs). Feed it a modest reinforcement instead and let
			//   the card's .1 crossover low-pass it.
			// PAN_SHARPNESS applications: square the raw (dot+1)/2 term so a
			//   sound actually localizes to its speaker instead of smearing
			//   at half-gain across the ones facing away. Squaring keeps
			//   on-axis (speakervol==1) at unity, so it doesn't dim sounds
			//   sitting on a speaker - only tightens the spread.
			const float SOLOUD_LFE_LEVEL = 0.35f;
			int j;
			for (j = 0; j < (signed)mChannels; j++)
			{
				...
				if (speaker[j].null())
				{
					speakervol = SOLOUD_LFE_LEVEL;
				}
				else
				{
					speakervol = (speaker[j].dot(pos) + 1) / 2;
					speakervol = speakervol * speakervol;
				}
				// Pan only. The mixer multiplies mChannelVolume by
				// mOverallVolume, which already contains m3dVolume (= vol,
				// set below) via updateVoiceVolume_internal - so including
				// vol here applied distance attenuation TWICE.
				v->mChannelVolume[j] = speakervol;
			}
			// </FS>
